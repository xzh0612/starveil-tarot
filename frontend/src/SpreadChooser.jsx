...
export function SpreadChooser({question,onQuestionChange,onConfirm}){
 const [result,setResult]=useState(null),[pending,setPending]=useState(false),[error,setError]=useState(''),[retry,setRetry]=useState(0),[all,setAll]=useState(false),[selected,setSelected]=useState(null),[custom,setCustom]=useState(['现状','阻碍','建议']);
 const query=question.trim();
 useEffect(()=>{
  setSelected(null);setResult(null);setError('');setPending(!!query);if(!query)return;
  ...
  const timer=setTimeout(async()=>{try{
   ...
   if(live)setResult({question:query,items:data.recommendations});
  }catch(e){if(live&&e.name!=='AbortError')setError(e.message);}finally{if(live)setPending(false);}},650);
  return()=>{live=false;clearTimeout(timer);controller.abort();};
 },[query,retry]);
 const change=value=>{if(value.trim()!==query){setSelected(null);setResult(null);setError('');}onQuestionChange(value);};
 ...
 return <div className="spread-chooser">
  ...
  {items.length>0&&<section aria-label="为你的问题推荐的牌阵"><div className="recommend-heading"><span className="eyebrow">为这个问题，推荐 {items.length} 个牌阵</span><small>先选一个，再开始抽牌</small></div><div className="recommended-spreads">{items.map((r,i)=>option(spreads.find(s=>s.id===r.id),r.reason,i))}</div></section>}
