...
function createEvidencePlan(evidence,cards,goals=[]){
 const items=Array.isArray(evidence)?evidence:[],cardItems=Array.isArray(cards)?cards:[],goalNames=[...new Set((Array.isArray(goals)?goals:[]).filter(Boolean))];
 return {perCard:cardItems.map(card=>{const matches=items.filter(item=>item?.cardId===card.id);return {cardId:card.id,position:card.position,anchorEvidenceIds:matches.filter(item=>item.retrievalRequired===true).map(item=>item.evidenceId),applicationEvidenceIds:matches.filter(item=>item.tier==='application').map(item=>item.evidenceId),referenceEvidenceIds:matches.filter(item=>item.tier==='reference').map(item=>item.evidenceId)};}),goalEvidenceIds:Object.fromEntries(goalNames.map(goal=>[goal,items.filter(item=>Array.isArray(item?.retrievalGoals)&&item.retrievalGoals.includes(goal)).map(item=>item.evidenceId)])),personalEvidenceIds:items.filter(item=>item?.tier==='personal').map(item=>item.evidenceId)};
}
...
export function buildReadingMessages(body,{evidenceOverride=null,includeRetrievalDiagnostics=false}={}){
 ...
 const context={question:body.question,activeQuestion,retrievalQuestion,queryMeta:{inheritedOriginal},spread,cards,evidence:evidence.map(promptEvidenceItem),evidenceMeta,evidencePlan,retrievalMeta,responsePlan,promptBudget,clarificationMeta:{allowClarification},safetyMeta:{requiresProfessionalBoundary:requiresBoundary},memoryEvidence};
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000,semanticReranker=null,semanticWeight=8,semanticTimeoutMs=1_500}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  if(debug){
   ...
   return reply(200,{source:'local',provider:'local',model,prompt:{systemChars:messages[0]?.content?.length??0,contextChars:contextContent.length,historyMessages:messages.slice(2).filter(message=>message.role!=='system').length},question:context.question,activeQuestion:context.activeQuestion,retrievalQuestion:context.retrievalQuestion,queryMeta:context.queryMeta,retrievalMeta:context.retrievalMeta,responsePlan:context.responsePlan,promptBudget:context.promptBudget,evidenceMeta:context.evidenceMeta,evidencePlan:context.evidencePlan,evidence:context.retrievalDiagnostics??context.evidence,promptEvidence:context.evidence,memoryEvidence:context.memoryEvidence});
  }
