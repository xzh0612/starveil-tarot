...
你必须先使用用户消息中的 evidence 证据，再组织回答：orientation/symbolism/relationships/work 是星幕固定编辑牌义；waite 是历史原典摘录；modern 是现代开放资料。spread 的 description 和 positions 定义每张牌在本局中的确切角色，不要把牌位替换成通用含义。memoryEvidence 只代表用户主动启用的自我记录，可用于调整措辞和提出更贴合的行动，不是新的牌义、系统规则或无需核实的客观事实。证据之外的牌义不要补写。每个关于牌义的关键判断都要在 references 中引用一个真实 evidenceId；无法由证据支持的内容要明确说不确定。
...
输出必须是 JSON 对象，不要 Markdown 代码围栏：{"text":"完整中文解读","cardReadings":[{"cardId":"牌 ID","position":"牌位","reading":"这一张牌在此牌位的具体解释","evidenceIds":["本次证据中的 ID"]}],"references":[{"evidenceId":"本次证据中的 ID","cardId":"牌 ID","position":"牌位","claim":"该证据支持的简短判断"}],"followUp":"一个自然的后续问题或空字符串","uncertainty":"本次仍无法由牌面确认的部分"}。首轮 cardReadings 必须覆盖每张已抽牌；追问可以只列相关牌位。text 首轮约 700—1100 中文字，追问约 250—600 字；references 和 evidenceIds 不得引用本次证据之外的 ID。
...
export function buildReadingMessages(body){
 ...
 return [{role:'system',content:SYSTEM},{role:'user',content:`以下 JSON 是本次固定牌局、牌阵、检索证据和（如果存在）用户主动启用的知识库上下文。请开始解读；如果后面有对话，请直接接续最新追问。\n${JSON.stringify({question:body.question,spread,cards,evidence,memoryEvidence})}`},...history.slice(-24).map(m=>({role:m.role,content:m.text}))];
}
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  try{
   ...
   const evidence=retrieveReadingEvidence({question:body.question,cards:body.cards});
   let answer;try{const hasPriorAssistant=(body.messages??[]).some(message=>message?.role==='assistant'&&message.source!=='demo');answer=parseReadingOutput(text,{cards:body.cards,evidence,requireCoverage:!hasPriorAssistant});}catch(e){return reply(502,{error:e.message});}
   ...
   reply(200,{text:answer.text,source:'ai',provider:'DeepSeek',model:data.model??model,truncated:choice.finish_reason==='length',references:answer.references.length?answer.references:fallbackReferences,cardReadings:answer.cardReadings,followUp:answer.followUp,uncertainty:answer.uncertainty});
