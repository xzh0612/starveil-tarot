...
export function buildReadingMessages(body,{evidenceOverride=null}={}){
 ...
 return [{role:'system',content:SYSTEM},{role:'user',content:`<starveil_context>\n${safeJson({question:body.question,activeQuestion,retrievalQuestion,queryMeta:{inheritedOriginal},spread,cards,evidence,evidenceMeta,retrievalMeta,responsePlan,promptBudget,clarificationMeta:{allowClarification},safetyMeta:{requiresProfessionalBoundary:requiresBoundary},memoryEvidence})}\n</starveil_context>`},...promptHistory.map(m=>({role:m.role,content:fenceHistoryMessage(m)}))];
}
