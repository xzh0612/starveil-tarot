...
export const READING_PROMPT_VERSION='nyx-prompt-v20';
...
function repairGuidance({code,evidence=[],cards=[],requiredGoalEvidence=[]}={}){
 ...
 const cardLines=(Array.isArray(cards)?cards:[]).map(card=>{
  ...
  :code==='action_text_support'
   ?'行动内容错误：每条 action（首轮或结构化追问）的正文必须复述所引证据中的具体、非通用概念，不能只写“行动/观察/结果”等通用词，也不能只挂 evidenceId。'
  :code==='action_reason_support'
   ?'行动理由错误：每条首轮或结构化追问 action 的 reason（若提供）必须复述所引证据中的具体、非通用概念，不能只写“建议/行动/观察”等通用词。'
