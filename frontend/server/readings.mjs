...
输出必须是 JSON 对象，不要 Markdown 代码围栏：{"text":"完整中文解读","needsClarification":false,"clarification":"需要澄清时只问一个具体问题，否则为空字符串","cardReadings":[{"cardId":"牌 ID","position":"牌位","reading":"这一张牌在此牌位的具体解释","evidenceIds":["本次证据中的 ID"]}],"synthesis":{"text":"把本局各张牌和牌位串联起来的综合判断","evidenceIds":["本次证据中的 ID"]},"actions":[{"text":"一条今天或本周可以执行并验证的动作","reason":"为什么这条动作与问题有关","evidenceIds":["本次证据中的 ID"]}],"references":[{"evidenceId":"本次证据中的 ID","cardId":"牌 ID 或 null","position":"牌位或 null","claim":"该证据支持的简短判断"}],"followUp":"一个自然的后续问题或空字符串","uncertainty":"本次仍无法由牌面确认的部分"}。首轮 synthesis 必须存在，且用牌面证据把所有已抽牌串联起来；synthesis.text 至少要复述所引证据中的一个有意义概念，不能只用“综合来看”等空泛话语；synthesis.evidenceIds 只能引用本次牌面 evidence，不能引用 personal 记忆。引用 personal 记忆时必须使用对应 memory evidenceId，并明确写 cardId:null、position:null，不能把记忆伪装成牌面。首轮 cardReadings 必须覆盖每张已抽牌，每张至少引用一个 retrievalRequired=true 的核心锚点；逐牌 reading 至少要复述一个所引证据中的具体概念或短语，不能只挂一个合法 evidenceId；并给出 1—3 条 actions；每条 action 必须有本次证据 ID、具体动作和可观察的完成标准；首轮 uncertainty 必须说明仍无法由牌面确认的部分。若 retrievalMeta 的 confidence 为 open 或 mixed，可以设置 needsClarification=true，此时 clarification 必须是一个具体澄清问题，cardReadings、synthesis、actions、references 可以为空；否则必须遵守首轮完整覆盖。追问可以只列相关牌位，actions 和 synthesis 可以为空。text 首轮按 responsePlan.targetText 输出（单牌约 700—1100 中文字，牌数增加时随计划递增），追问约 250—600 字；references、cardReadings.evidenceIds、synthesis.evidenceIds 和 actions.evidenceIds 不得引用本次证据之外的 ID。
