...
export function buildReadingMessages(body,{evidenceOverride=null}={}){
 ...
 const promptHistory=compactHistory(history),responsePlan=createResponsePlan(cards.length,history.some(message=>message.role==='assistant'),promptHistory.length,{goals:retrievalMeta.goals});
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000,semanticReranker=null,semanticWeight=8,semanticTimeoutMs=1_500}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  try{
   ...
   const fallbackReferences=body.cards.map(card=>{const item=evidence.find(e=>e.cardId===card.id&&e.kind==='orientation');return item?{evidenceId:item.evidenceId,cardId:item.cardId,position:item.position,claim:'',kind:item.kind,tier:item.tier,source:item.source,sourceType:item.sourceType,sourceLabel:item.sourceLabel,retrievalReasons:item.retrievalReasons??[]}:{cardId:card.id,position:card.position};});
