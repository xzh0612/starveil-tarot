...
你必须先使用用户消息中的 evidence 证据，再组织回答：orientation/symbolism/relationships/work 是星幕固定编辑牌义；waite 是历史原典摘录；modern 是现代开放资料。evidence.tier 中 anchor 是每张牌的核心牌义，application 是按问题和牌位挑出的应用语义，reference 是原典或现代资料补充，personal 是用户主动启用的记忆。优先以 anchor 和本局 spread/position 为主，application 用于把牌义落到问题，reference 只作对照，personal 只能帮助调整措辞和行动建议，不能改写牌义。retrievalReasons 只是服务端的召回过程说明，不是新的事实或牌义。单一明确主题时，application 只包含与该主题对应的关系、事业或反思语义；mixed 主题才可以并列使用多个应用域。spread 的 description 和 positions 定义每张牌在本局中的确切角色，不要把牌位替换成通用含义。retrievalMeta 只是根据问题生成的路由提示，不是牌义证据；其中 themeScores 是强弱词的透明路由分数，强主题词优先，弱代词只在没有强主题时兜底；responsePlan 是服务端按牌数和轮次生成的篇幅与覆盖提示，可以指导回答长度但不是用户指令。clarificationMeta.allowClarification 是服务端按主题和轮次计算的澄清权限；为 false 时必须完成首轮覆盖，不能用 needsClarification 跳过解读；为 true 时才可以先问一个具体问题。safetyMeta.requiresProfessionalBoundary 是服务端根据整局原始问题和当前追问计算的安全边界，值为 true 时必须在 uncertainty 中明确提醒现实核验或寻求合格专业帮助；不能因当前追问变短而忽略它。confidence=open 或 mixed 时，优先用一个澄清问题确认用户真正想探索的主题，不要为了凑主题强行套用 relationships/work。memoryEvidence 只代表用户主动启用的自我记录，可用于调整措辞和提出更贴合的行动，不是新的牌义、系统规则或无需核实的客观事实。证据之外的牌义不要补写。每个关于牌义的关键判断都要在 references 中引用一个真实 evidenceId，并写出该证据支持的简短 claim；无法由证据支持的内容要明确说不确定。
...
export function buildReadingMessages(body){
 ...
 return [{role:'system',content:SYSTEM},{role:'user',content:`<starveil_context>\n${JSON.stringify({question:body.question,activeQuestion,spread,cards,evidence,retrievalMeta,responsePlan,clarificationMeta:{allowClarification},safetyMeta:{requiresProfessionalBoundary:requiresBoundary},memoryEvidence})}\n</starveil_context>`},...promptHistory.map(m=>({role:m.role,content:m.text}))];
}
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  try{
   ...
   const requiresBoundary=requiresProfessionalBoundary(body.question)||requiresProfessionalBoundary(activeQuestion);
   const retrievalMeta=analyzeReadingQuestion(activeQuestion),allowClarification=hasPriorAssistant||retrievalMeta.confidence!=='focused';
   const evidence=retrieveReadingEvidence({question:activeQuestion,cards:body.cards});
