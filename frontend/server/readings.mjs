...
function createEvidencePlan(evidence,cards,goals=[]){
 ...
 return {perCard:cardItems.map(card=>{const matches=items.filter(item=>item?.cardId===card.id);return {cardId:card.id,position:card.position,anchorEvidenceIds:matches.filter(item=>item.retrievalRequired===true).map(item=>item.evidenceId),applicationEvidenceIds:matches.filter(item=>item.tier==='application').map(item=>item.evidenceId),referenceEvidenceIds:matches.filter(item=>item.tier==='reference').map(item=>item.evidenceId)};}),goalEvidenceIds:Object.fromEntries(goalNames.map(goal=>[goal,goalEvidence(goal).map(item=>item.evidenceId)])),goalRequiredEvidenceIds:Object.fromEntries(goalNames.map(goal=>[goal,goalEvidence(goal).filter(item=>item.tier===GOAL_EVIDENCE_TIERS[goal]).map(item=>item.evidenceId)])),personalEvidenceIds:items.filter(item=>item?.tier==='personal').map(item=>item.evidenceId)};
}
...
export function buildReadingMessages(body,{evidenceOverride=null,includeRetrievalDiagnostics=false}={}){
 ...
 const promptHistory=compactHistory(history),promptBudget=createPromptBudget(history,promptHistory,{question:body.question,activeQuestion,evidence,memoryEvidence}),responsePlan=createResponsePlan(cards.length,history.some(message=>message.role==='assistant'),promptHistory.length,{goals:retrievalMeta.goals});
 const allowClarification=history.some(message=>message.role==='assistant')||retrievalMeta.confidence!=='focused';
 const evidencePlan=createEvidencePlan(evidence,cards,retrievalMeta.goals);
 const knowledgeMeta={deckVersion:DECK_VERSION,ragVersion:READING_KNOWLEDGE_VERSION,clientDeckVersion:typeof body.deckVersion==='string'?body.deckVersion:null};
 const context={question:body.question,activeQuestion,retrievalQuestion,queryMeta:{inheritedOriginal},spread,cards,evidence:evidence.map(promptEvidenceItem),evidenceMeta,evidencePlan,retrievalMeta,responsePlan,promptBudget,knowledgeMeta,clarificationMeta:{allowClarification},safetyMeta:{requiresProfessionalBoundary:requiresBoundary},memoryEvidence};
 if(includeRetrievalDiagnostics)context.retrievalDiagnostics=evidence;
 const turnReminder=`<starveil_turn>\n${safeJson({activeQuestion,turn:responsePlan.turn,goal:responsePlan.goal,cardIds:cards.map(card=>card.id),positions:cards.map(card=>card.position)})}\n</starveil_turn>\n请只按 system contract 回应当前轮次。`;
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000,semanticReranker=null,semanticWeight=8,semanticTimeoutMs=1_500}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  if(debug){
   ...
   return reply(200,{source:'local',provider:'local',model,prompt:{systemChars:messages[0]?.content?.length??0,contextChars:contextContent.length,historyMessages:messages.slice(2).filter(message=>message.role!=='system').length},question:context.question,activeQuestion:context.activeQuestion,retrievalQuestion:context.retrievalQuestion,queryMeta:context.queryMeta,retrievalMeta:context.retrievalMeta,responsePlan:context.responsePlan,promptBudget:context.promptBudget,knowledgeMeta:context.knowledgeMeta,evidenceMeta:context.evidenceMeta,evidencePlan:context.evidencePlan,evidence:context.retrievalDiagnostics??context.evidence,promptEvidence:context.evidence,memoryEvidence:context.memoryEvidence});
  }
