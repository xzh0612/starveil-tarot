...
import {parseReadingOutput,retrieveMemoryEvidence,retrieveReadingEvidence,requiresProfessionalBoundary} from './reading-rag.mjs';
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  try{
   ...
   const evidence=retrieveReadingEvidence({question:body.question,cards:body.cards});
   let answer;try{const hasPriorAssistant=(body.messages??[]).some(message=>message?.role==='assistant'&&message.source!=='demo');answer=parseReadingOutput(text,{cards:body.cards,evidence,requireCoverage:!hasPriorAssistant,requireUncertainty:requiresProfessionalBoundary(body.question)});}catch(e){return reply(502,{error:e.message});}
