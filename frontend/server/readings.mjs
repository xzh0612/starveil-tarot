...
export const READING_PROMPT_VERSION='nyx-prompt-v44';
...
function createPromptBudget(history,promptHistory,{question='',activeQuestion='',evidence=[],memoryEvidence=[],historyLimits={}}={}){
 ...
 return {historyInputMessages:source.length,historySelectedMessages:selected.length,historyInputChars:chars(source),historySelectedChars:chars(selected),historyOmittedMessages:Math.max(0,source.length-selected.length),historyTruncatedMessages:source.filter(item=>String(item?.text??'').length>maxMessageChars).length,historyLimits:{maxMessages,maxMessageChars,maxTotalChars},promptCharLimit:READING_PROMPT_CHAR_LIMIT,historyCompactedForBudget:maxTotalChars<24_000,questionChars:String(question??'').length,activeQuestionChars:String(activeQuestion??'').length,evidenceItems:Array.isArray(evidence)?evidence.length:0,evidenceTextChars:chars(evidence),memoryItems:memories.length,memoryTextChars:chars(memories),memoryExcerptedItems:memories.filter(item=>item?.memoryExcerpted===true).length,memoryLimits:{maxItems:6,maxItemChars:2_000,maxTotalChars:6_000}};
}
