...
export const READING_PROMPT_VERSION='nyx-prompt-v1';
...
export function createReadingMiddleware({apiKey,model='deepseek-flash',fetchImpl=fetch,timeoutMs=90000,semanticReranker=null,semanticWeight=8,semanticTimeoutMs=1_500}={}){
 ...
 return async function readingMiddleware(req,res,next){
  ...
  try{
   ...
   reply(200,{text:answer.text,source:'ai',provider:'DeepSeek',model:provider.data.model??model,promptVersion:READING_PROMPT_VERSION,truncated:provider.choice.finish_reason==='length',references:answer.references.length?answer.references:fallbackReferences,cardReadings:answer.cardReadings,synthesis:answer.synthesis,goalSections:answer.goalSections,actions:answer.actions,needsClarification:answer.needsClarification,clarification:answer.clarification,followUp:answer.followUp,uncertainty:answer.uncertainty,evidenceMeta,goalPlan});
