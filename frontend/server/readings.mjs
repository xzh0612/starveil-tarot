...
export const READING_PROMPT_VERSION='nyx-prompt-v24';
...
function repairCode(error){
 ...
 const rules=[
  ...
  [/首轮正文没有覆盖每个回答目标/u,'mixed_text_goal_coverage'],
...
function repairGuidance({code,evidence=[],cards=[],requiredGoalEvidence=[]}={}){
 ...
 const cardLines=(Array.isArray(cards)?cards:[]).map(card=>{
  ...
  :code==='mixed_text_goal_coverage'
   ?'混合目标正文错误：首轮 text 必须分别覆盖每个 goalSections 目标，至少复述该目标证据中的一个具体概念；不能只写其中一个目标。'
  :code==='text_support'
