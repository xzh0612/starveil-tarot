...
export const READING_PROMPT_VERSION='nyx-prompt-v41';
...
当 responsePlan.goal=open 且 clarificationMeta.allowClarification=false 时，主题虽然明确但目标尚未明确；只能给核心牌面线索、现实观察和用户可控制的动作，不得输出复合、联系、成功等预测或他人内心判断。 当 activeQuestion 明确包含关系、事业或自我主题时，顶层 text 还必须直接回应每个明确主题，并为每个主题复述至少一个主题词；开放或只有选择词的问题没有明确主题时不要强行添加主题。
...
function repairGuidance({code,evidence=[],cards=[],requiredGoalEvidence=[],missingGoalCoverage=[]}={}){
 ...
 const focus=code==='missing_position_evidence'
  ...
  :code==='open_goal_prediction'
   ?'开放目标错误：主题虽然明确，但用户没有指定预测目标；不要输出会复合、会回来、会联系、会成功等预测或他人内心判断。请改为核心牌面线索、现实观察和用户可控制的记录动作。'
