...
export const READING_PROMPT_VERSION='nyx-prompt-v27';
...
function createResponsePlan(cardCount,hasPriorAssistant,historyMessages=0,{goals=[]}={}){
 ...
 const goal=goals.length===1?goals[0]:goals.length>1?'mixed':'open',emphasis=RESPONSE_EMPHASIS[goal],directAnswer=RESPONSE_DIRECTIVES[goal];
 if(hasPriorAssistant)return {turn:'followup',cardCount:count,historyMessages,targetText:'250—600 中文字',goal,emphasis,directAnswer,requireCardCoverage:false,requireActions:false,requireReferences:false,requireSynthesis:false};
 const min=700+(count-1)*90,max=1100+(count-1)*140;
 return {turn:'first',cardCount:count,historyMessages,targetText:`${min}—${max} 中文字`,goal,emphasis,directAnswer,requireCardCoverage:true,requireActions:true,requireReferences:true,requireSynthesis:true};
}
...
export function buildReadingMessages(body,{evidenceOverride=null,includeRetrievalDiagnostics=false}={}){
 ...
 const turnReminder=`<starveil_turn>\n${safeJson({activeQuestion,turn:responsePlan.turn,goal:responsePlan.goal,directAnswer:responsePlan.directAnswer,goalOrder:goalPlan.order,cardIds:cards.map(card=>card.id),positions:cards.map(card=>card.position)})}\n</starveil_turn>\n请只按 system contract 回应当前轮次。`;
