...
function repairGuidance({code,evidence=[],cards=[],requiredGoalEvidence=[]}={}){
 ...
 const cardLines=(Array.isArray(cards)?cards:[]).map(card=>{
  ...
  :code==='goal_alignment'
   ?'回答目标错误：按本轮 responsePlan.goal 回答；advice 要落到下一步行动，forecast 要区分趋势与现实核验，explanation 要说明原因或牌面线索，comparison 要并列条件、代价或取舍；mixed 时每个 goalSections 分段都要遵守自己的目标。'
