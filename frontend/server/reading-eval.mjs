...
export function evaluateReadingFixture({question,cards,output,requiredKinds=[]}={}){
 ...
 try{
  ...
  const requiredGoalEvidence=routing.goals.filter(goal=>['forecast','comparison'].includes(goal)&&evidenceMeta.goalCoverage[goal]?.ok),requireGoalSections=routing.goals.length>1;
