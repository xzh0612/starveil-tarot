...
export function evaluateReadingFixture({question,cards,output,requiredKinds=[]}={}){
 ...
 try{
  parsed=parseReadingOutput(output,{cards,evidence:retrieval.evidence,requireCoverage:true,requireActions:true,requireReferences:true,requireReferenceClaims:true,requireReferenceSupport:true,requireCardReadingSupport:true,requireSynthesis:true,requireUncertainty:true,requireRealityBoundary:requiresProfessionalBoundary(question),allowClarification});
 }catch{
