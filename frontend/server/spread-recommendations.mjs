...
export function parseRecommendations(text){
 ...
 return items.map(r=>{if(!r||!spreads.some(s=>s.id===r.id)||seen.has(r.id)||typeof r.reason!=='string'||!r.reason.trim()||r.reason.length>300)throw Error('牌阵推荐内容无效，请重试。');seen.add(r.id);return {id:r.id,reason:r.reason.trim()};});
}
