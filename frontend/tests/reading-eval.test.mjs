...
test('reading evaluation catches a high-stakes action without reality verification',()=>{
 const question='这项投资要不要买？';
 const card=cards[0];
 const evidence=retrieveReadingEvidence({question,cards:[card]});
 const output=JSON.stringify({
  text:'稳定、温柔而明确的条件可以帮助比较。',
  synthesis:{text:'稳定与克制提示先整理条件。',evidenceIds:['m08:orientation']},
  cardReadings:[{cardId:'m08',position:'建议',reading:'用稳定、温柔而明确的方式面对冲动。',evidenceIds:['m08:orientation']}],
  actions:[{text:'今天依据稳定、明确的牌面直接买入。',reason:'把稳定、明确的克制转成行动。',evidenceIds:['m08:orientation']}],
  references:[{evidenceId:'m08:orientation',cardId:'m08',position:'建议',claim:'稳定、明确'}],
  uncertainty:'当前缺少比较条件与代价证据，需要核实产品资料。',
 });
 const result=evaluateReadingFixture({question,cards:[card],output});
 assert.equal(result.ok,false);
 assert.ok(result.issues.includes('output_contract'));
});
...
test('prompt evaluation requires evidence boundaries, JSON contract and user context',()=>{
 ...
 const missingProfessionalActionRule=[...messages];
 missingProfessionalActionRule[0]={...messages[0],content:messages[0].content.replace('高风险 action 若涉及买入、卖出、下单、签约、起诉、停药、用药、转账或借贷，必须先核实资料、评估风险或咨询专业人士；','')};
 assert.ok(evaluatePromptContract(missingProfessionalActionRule).issues.includes('missing_professional_action_rule'));
 const missingOpenGoalRule=[...messages];
 missingOpenGoalRule[0]={...messages[0],content:messages[0].content.replace('responsePlan.goal=open 且 clarificationMeta.allowClarification=false 时只能给核心牌面线索和现实观察，不得输出复合、联系或成功等预测；','')};
 assert.ok(evaluatePromptContract(missingOpenGoalRule).issues.includes('missing_open_goal_boundary_rule'));
...
test('prompt evaluation protects truncated personal-memory excerpts',()=>{
 ...
 missing[1].content=missing[1].content.replaceAll('memoryExcerpted','');
 const result=evaluatePromptContract(missing);
 assert.ok(result.issues.includes('missing_memory_excerpt_rule'));
 assert.ok(result.issues.includes('missing_memory_excerpt_context'));
});
