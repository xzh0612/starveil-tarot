import {analyzeReadingQuestion,canAskClarification,parseReadingOutput,requiresPerspectiveBoundary,requiresProfessionalBoundary,retrieveReadingEvidence,summarizeReadingEvidence} from './reading-rag.mjs';
...
export function evaluatePromptContract(messages=[]){
 ...
 if(!/goalCoverageByCard[\s\S]{0,500}missingGoalCoverageByCard[\s\S]{0,500}coverageBoundaryGoals/i.test(system))issues.push('missing_per_card_goal_coverage_rule');
 ...
 if(!/(?:确定日期|确定时间|具体日期)/u.test(system))issues.push('missing_timing_calibration_rule');
 ...
 if(!/safetyMeta/.test(user)||!/requiresPerspectiveBoundary/.test(user))issues.push('missing_perspective_boundary_context');
 ...
 !issues.includes('missing_goal_coverage_rule'),
  !issues.includes('missing_per_card_goal_coverage_rule'),
  ...
  !issues.includes('missing_perspective_boundary_context'),
