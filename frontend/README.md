...
首轮解读必须返回至少一条 `actions`；服务端会校验每条行动的 `evidenceIds`，并在健康、法律、投资等高风险问题缺少 `uncertainty` 现实边界时拒绝该响应。若首轮 JSON 不合约，后端最多追加一次只针对结构修复的请求，再走同一套牌面、证据和安全校验；追问可以只返回相关牌位，行动清单也可以为空。
