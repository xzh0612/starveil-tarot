...
Prompt v41 在 v40 的高风险行动边界之外，继续把“主题明确但目标未明确”落成可验证门禁：首轮 `responsePlan.goal=open` 且不允许澄清时，只能给核心牌面线索、现实观察和用户可控制的记录动作，不能擅自输出会复合、会回来、会联系、会成功等结果预测或他人内心判断；解析器与离线评估都会返回稳定的 `open_goal_prediction` 代码并触发一次有边界的修复。高风险 action 仍必须先安排核实现实资料、评估风险或咨询合格专业人士，不能直接要求执行不可逆决定。
