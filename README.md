...
RAG v39 在 v38 的应用域隔离和个人记忆字符预算之外，为知识库召回增加了保守的中文同义词扩展：只有问题先命中具体语义组时才允许低权重扩展（例如“交流”补充“沟通”），无关记录仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
