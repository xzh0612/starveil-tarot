...
RAG v41 在 v40 的应用域隔离、个人记忆字符预算、保守同义词扩展和行动后缀路由之外，补齐了“怎么选／怎么选择”的比较意图，使多个方案的问题进入比较 application 层，而“如何兼顾”仍留在建议层。只有接着发展、变化、结果等趋势语义时才进入预测目标，避免检索错误的 reference 层。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
