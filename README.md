...
它不调用 DeepSeek，只用固定问题、牌面和模拟 JSON 检查证据主题覆盖、逐牌覆盖、核心锚点、正文和行动理由的证据支持、引用合法性、可执行建议、覆盖不足边界、选择型问题的决策应用召回、明确“哪条/哪种”比较目标路由、目标化 responsePlan、混合问题的 goalPlan 逐目标顺序、goalSections 分段校验与证据绑定、目标证据覆盖诊断与覆盖不足边界、大牌阵动态证据预算与目标层级预留、弱代词追问的原始主题继承、具体引用词校验、绝对断言校准、澄清分支边界和 Prompt 上下文边界、模型证据与检索诊断隔离。当前共有 160 个测试，离线 Prompt、RAG 检索和结构化解读评估均需通过。
