...
密钥只能放在 `DEEPSEEK_API_KEY`，不要使用 `VITE_` 前缀。接口只允许本机调用，并会校验牌面、正逆位、请求大小、来源、并发和超时。模型输出的行动建议必须引用本局证据；健康、法律、投资等高风险问题还必须包含现实依据边界。若 JSON 结构偶发不合约，后端最多自动修复一次并重新校验。档案与个人知识库不会自动发送给模型。
