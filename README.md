...
RAG v54 在 v49 的通用解释证据隔离、个人记忆短词保护、口语比较、因果路由和 perspective boundary 之外，补齐了胸口疼、头疼、停药、换药、起诉、买房、睡眠不足等口语高风险表达的现实边界；普通“焦虑/辞职”仍不会被自动扩大为专业风险；同时把“他到底在想什么”“对方是什么心态”“她什么意思”等第三人称内心状态口语统一纳入 perspective boundary，要求把核验落到沟通和可观察行为；v52 将个人记忆召回方法名抽成共享契约常量，让 Prompt 与实际检索器不会再次漂移；并补上“他对我态度如何”“对方怎么看我”等关系视角口语边界；这些表达现在同时路由到 explanation，能拿到对应的核心证据层。

Prompt v46 在 v44 的开放目标门禁之外，继续覆盖“有戏、有没有机会、有没有结果、对方有没有感觉”等口语化结果预测：首轮 `responsePlan.goal=open` 且不允许澄清时，只能给核心牌面线索、现实观察和用户可控制的记录动作，不能通过换词擅自输出结果预测或他人内心判断；解析器与离线评估都会返回稳定的 `open_goal_prediction` 代码并触发一次有边界的修复。高风险 action 仍必须先安排核实现实资料、评估风险或咨询合格专业人士，不能直接要求执行不可逆决定。Prompt 总字符预算约为 56,000；当大牌阵与长历史接近上限时，服务端优先保留当前证据和最近一轮，只进一步压缩更早历史，并在 `promptBudget.historyLimits` 中记录实际预算。 单条过长历史会采用开头加结尾的压缩方式，并用省略标记隔开中段，避免丢掉末尾的行动和边界信息。
