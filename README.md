...
RAG v46 在 v45 的应用域隔离、个人记忆字符预算、保守同义词扩展、口语比较、因果问法和否定预测识别之外，补齐了“是因为工作太忙吗”“是什么导致现在的状态”等因果表达：它们进入 explanation 目标并召回原因与牌面线索，普通“是不是会有变化”仍保持 forecast。双选项结果问题仍同时保留 forecast 与 comparison，避免把选择问题误写成单一路径趋势。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
