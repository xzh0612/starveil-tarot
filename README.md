...
RAG v47 在 v46 的应用域隔离、个人记忆字符预算、口语比较、因果问法和否定预测识别之外，收紧了通用解释问题的位置证据：`这张牌是什么意思` 只保留 anchor/reference，不再因为“建议”或“关系挑战”等牌位文字推断 work/relationships application；明确的关系、自我主题以及 forecast 的位置行动语义仍按既有规则保留。因果表达继续进入 explanation，双选项结果问题继续同时保留 forecast 与 comparison。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，单独的“建议”牌位不会凭空制造事业语境。
