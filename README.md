...
RAG v38 在 v37 的应用域隔离之外，为个人记忆增加了确定性的总字符预算，超出预算的记忆会标记为截断片段，模型不得补写省略内容。没有明确关系或事业主题的建议／比较问题只保留反思应用资料；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
