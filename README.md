...
RAG v48 在 v47 的通用解释证据隔离、个人记忆字符预算、口语比较、因果问法和否定预测识别之外，收紧了个人记忆的短词召回：代词、时间词和功能性短语（如“我最/最近”）不会单独暴露记录，只有具体语义词或足够多的独立短词才能进入 Prompt；同义词扩展和长记忆截断规则保持不变。通用解释仍只保留 anchor/reference，明确主题与 forecast 的位置语义按既有规则保留。
