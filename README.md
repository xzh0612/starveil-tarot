...
RAG v40 在 v39 的应用域隔离、个人记忆字符预算和保守同义词扩展之外，修正了“关系如何处理”“感情和工作如何平衡”这类问题的目标路由：行动后缀会保留建议目标，只有接着发展、变化、结果等趋势语义时才进入预测目标，避免检索错误的 reference 层。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
