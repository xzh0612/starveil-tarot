...
RAG v45 在 v44 的应用域隔离、个人记忆字符预算、保守同义词扩展、因果问法和否定预测识别之外，补齐了“是选 A 还是选 B”“我应该选哪个方案”“哪一种发展更适合我”等口语比较表达：它们进入 comparison 目标并召回选项、条件与取舍证据，同时保留真正存在的 forecast 语义。因果问法仍进入 explanation，双选项结果问题仍同时保留 forecast 与 comparison，避免把选择问题误写成单一路径趋势。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
