...
RAG v43 在 v42 的应用域隔离、个人记忆字符预算、保守同义词扩展、行动后缀路由、选择意图和否定预测识别之外，补齐了“如果选择 A 会怎样，选择 B 会怎样”这类双选项结果问题：保留 forecast 的结果语义，同时加入 comparison 的选项、条件与取舍证据，避免只给单一路径的趋势解读。只有接着发展、变化、结果等趋势语义时才进入预测目标，避免检索错误的 reference 层。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
