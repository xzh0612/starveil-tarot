...
RAG v44 在 v43 的应用域隔离、个人记忆字符预算、保守同义词扩展、行动后缀路由、选择意图和否定预测识别之外，补齐了“是不是因为/是否由于/会不会是因为”这类因果问法：它们进入 explanation 目标并召回原因、牌面线索证据，而“是不是会有变化”仍保留 forecast。双选项结果问题仍同时保留 forecast 与 comparison，避免把原因问题误写成结果预测或把路径比较缩成单一路径趋势。无关记忆仍不会进入 Prompt；真正开放的问题只保留核心牌义和参考资料，只有牌位本身明确是关系或自我反思角色时才加入对应应用资料。单独的“建议”牌位不会再凭空制造事业语境，明确的预测问题仍可使用牌位挑出的行动语义。
